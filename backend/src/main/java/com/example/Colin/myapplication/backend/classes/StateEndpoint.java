...
public class StateEndpoint {
    ...
    @ApiMethod(
            ...
            path = "state",
            httpMethod = ApiMethod.HttpMethod.GET)
    public CollectionResponse<State> list(@Nullable @Named("cursor") String cursor, @Nullable @Named("limit") Integer limit) {
        limit = limit == null ? DEFAULT_LIST_LIMIT : limit;
        Query<State> query = ofy().load().type(State.class).limit(limit);
        if (cursor != null) {
            query = query.startAt(Cursor.fromWebSafeString(cursor));
        }
        QueryResultIterator<State> queryIterator = query.iterator();
        List<State> stateList = new ArrayList<State>(limit);
        while (queryIterator.hasNext()) {
            stateList.add(queryIterator.next());
        }
        return CollectionResponse.<State>builder().setItems(stateList).setNextPageToken(queryIterator.getCursor().toWebSafeString()).build();
    }
